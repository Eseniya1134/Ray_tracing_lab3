...
void initializeDefaultScene(out STriangle triangles[12], out SSphere spheres[2]) {
    // Инициализация комнаты (6 граней куба по 2 треугольника каждая)
    // Левая стена
...
void initializeDefaultLightMaterials(out SLight light, out SMaterial materials[10]) {
    ...
    materials[3].Color = vec3(1.0, 1.0, 1.0);  // Потолок - белый
    ...
    materials[4].Color = vec3(1.0, 1.0, 1.0);  // Пол - белый
    ...
    materials[5].Color = vec3(1.0, 1.0, 1.0);  // Задняя стена - белая
...
bool Raytrace(SRay ray, float start, float final, inout SIntersection intersect) {
    ...
    // Проверка пересечения с треугольниками комнаты
    for(int i = 0; i < 12; i++) {
        ...
        if(IntersectTriangle(ray, triangle.v1, triangle.v2, triangle.v3, test) && test < intersect.Time) {
            ...
            // Используем материал для всей грани (2 треугольника на грань)
    ...
    for(int i = 0; i < 4; i++) {
        ...
        if(IntersectTriangle(ray, triangle.v1, triangle.v2, triangle.v3, test) && test < intersect.Time) {
            ...
            SMaterial mat = Materials[8]; // Материал тетраэдра
...
void main() {
        ...
        if (Raytrace(ray, start, final, intersect)) {
            switch(intersect.MaterialType) {
                ...
                case MIRROR_REFLECTION: {
                    // Добавляем диффузную составляющую, если коэффициент отражения < 1
    ...
    // Убедимся, что цвет находится в допустимом диапазоне
